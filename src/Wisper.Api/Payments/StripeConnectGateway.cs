...
/// <summary>
/// Real <see cref="IStripeConnectGateway"/> over the Stripe SDK (docs/PAYMENTS.md §5). Each call constructs the
/// relevant Stripe service from the shared <see cref="IStripeClient.Sdk"/>. The connected account is created
/// <b>Express</b> with the <c>transfers</c> capability requested — Wisper uses separate charges &amp; transfers,
/// so the platform holds funds and pays hosts by Transfer to their connected balance (docs/PAYMENTS.md §1).
