...
| `wisp_version` / `agent_version` | `text` | intended to mirror `hello.wispVersion`/`agentVersion`; **not populated today** (the tunnel does not write them), so they read NULL |
| `max_leases` / `max_streams` | `int` | intended for the top-level `hello.capacity{maxLeases,maxStreams}`; **not populated and not enforced today** (`TUNNEL.md` §5). Per-host admission uses the live `capability.capacity.max_contracts` snapshot instead |
...
**(e) Reconciliation** re-derives `SUM` per account from `ledger_entries` and compares it to `balance_cents`; any drift should page an operator. Balances are a cache; the journal is truth. *Status:* the derivation exists in code (`LedgerService.ReconcileAsync`) but **no scheduled job or alert runs it yet**; it has to be invoked by hand.
...
| `created_at` / `expires_at` | `timestamptz` NOT NULL | TTL = 24 h; an expired row is swept lazily when its key is presented again (no scheduled sweep runs) |
...
- **Reconciliation.** The balance re-derivation (§7e) is the intended safety net that turns any silent drift into a page, not a loss; it is not scheduled yet.
