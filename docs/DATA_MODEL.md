...
| `isolation_levels` | `text[]` NOT NULL DEFAULT `ARRAY['shared']` | effective isolation levels the host offers, from the `hello`/`heartbeat` capability (`TUNNEL.md` §5); surfaced in `GET /v1/catalog`. Stored as free-form `text` (like `host_images.networks`), not an enum. A hello / heartbeat with an **absent capability block, or a present block whose `isolation_levels` is absent or empty**, leaves the persisted list untouched (task #191): an agent cannot legitimately advertise zero tiers, so a kata/gVisor host reconnecting through a wisp blip is not overwritten with the DB default. The DB default is what surfaces only for a first-ever hello on a fresh row (nothing was persisted yet). **The persisted row is not the live snapshot** (task #206): a hello whose top-level `capability` block is absent keeps the persisted isolation here but resets the live capability snapshot for that tunnel session to `null` until the next heartbeat carries one; while it is `null`, `GET /v1/catalog` surfaces `os` / `max_leases` as `null` for the host and `PUT/PATCH /v1/hosts/:id/images` upserts fail `409 host_offline`. Migration `0010_HostIsolation` |
| `default_isolation` | `text` NOT NULL DEFAULT `'shared'` | level applied when a lease omits `isolation`. Follows the same "absent = keep last known" rule as `isolation_levels` (task #191). Migration `0010_HostIsolation` |
...
**(f) Admin lookup.** `GET /v1/admin/ledger/accounts` (`API.md` §8, task #194) exposes the `ledger_accounts` table as a paged listing (`kind`/`owner_user_id` filters, `limit`/`offset` paging ordered by `created_at, id`) so an operator can find the two account ids `POST /v1/admin/adjustments` requires (a `platform_revenue` singleton and a user's wallet) without opening the database. Each row carries the account's `id`, `kind`, `owner_user_id`, `currency`, `balance_cents`, plus the joined `users.email` as `owner_email` (`null` for platform singletons whose owner is NULL). Read-only, admin-role-gated, and (like the other admin *reads*) not audited.
...
**Fail-safe (task #184, task #202).** Even with the seed, the paths that consume `platform_policy` refuse to proceed silently, but they also refuse to STRAND an active lease if a policy is somehow still missing at end time. The chosen rule: `POST /v1/leases` reads the active policy up front and, when none is present, refuses the create with a clear error (before any wallet hold or tunnel frame) rather than provision a lease no `lease_charge` can be split against. The metering flush (used by the periodic tick and every finalize driver: consumer DELETE, admin force-end, suspend / container-lost / `lease.ended`) resolves the policy through a two-step fallback instead of throwing (task #202): (1) the active version (newest `effective_from <= now`); (2) if none is active, the newest version regardless of `effective_from`, with the flush logging at Error under the `billing.policy.stale_fallback` event and splitting the charge against that version so money accounting stays correct; (3) if truly no policy row exists at all (impossible after `0017` but kept as a guard), the flush logs `billing.policy.missing_at_flush` at Critical as a billing-integrity alert, skips the `lease_charge`, and returns null so the caller's transition-to-ended + `hold_release` still runs. The full hold returns to the wallet and no lease is left active with a parked hold. On either fallback branch the flush also writes one `billing_incidents` row (task #210, §17) so `GET /v1/admin/overview` surfaces the incident durably across restarts and instances; `POST /v1/admin/policy/fallback/ack` clears the badge by shifting the ack watermark without erasing history. The periodic tick additionally wraps each per-lease flush in its own try/catch (task #202): any transient exception on one lease (the fee split, a ledger post, a repository read) is logged with the lease id and skipped, `last_metered_at` stays behind on that lease, and the next tick retries the same interval, so the ledger idempotency on `(lease_id, period_start)` guarantees no double-charge.
...
| `action` | `text` | as written today: `policy.update`, `host.suspend`/`host.unsuspend`, `user.suspend`/`user.unsuspend`, `admin.refund`, `billing.refund` (the self-serve refund, actor = the consumer), `ledger.adjustment`, `lease.admin_end`, `user.chargeback_suspend` (system actor, NULL), and `payout.settled`/`payout.failed`/`payout.rejected` (target = the host user; actor = the host on the on-demand path or NULL for the scheduled system run; meta carries `payout_id`, `amount_cents`, `currency`, `host_ids` (the caller's hosts at payout time, task #203), and `stripe_transfer_id`/`ledger_txn_id` on settlement or the transfer `error` on failure; `payout.rejected` covers on-demand pre-transfer rejections (`reason` = `connect_incomplete`/`below_minimum`/`no_connect_account`) so operators see why a self-serve run did not move money; scheduled below-minimum skips stay unaudited by design to avoid per-run noise since earnings are untouched; `trigger` = `on_demand`/`scheduled`; tasks #185, #203, so a reviewer has the same trail refunds and chargebacks leave). Post-hoc audit appends after a successful transfer are best-effort: a failure is logged with the payout id but does not bubble to the caller, so a stray audit-write cannot 500 a payout that already moved money. |
...
## 17. Billing incidents & operational state (task #210)

Durable operator-visible state that the earlier tables never held: signals that ride outside the money-move flows themselves but that an operator has to act on. Introduced by migration `0018_BillingIncidents` to persist the platform-policy fallback signal (`PAYMENTS.md` §4) so the badge on `GET /v1/admin/overview` survives restarts and reads the same from any instance. Two tables:

### `billing_incidents`

Append-only journal of one row per observed billing-integrity incident. The metering flush (`MeteringService.ResolvePolicyForFlushAsync`, `PAYMENTS.md` §4) writes here on either fallback branch: `kind = 'policy_stale_fallback'` when no active `platform_policy` row resolved but a version existed (the flush split against the newest version) or `kind = 'policy_missing_at_flush'` when no row existed at all (impossible after migration `0017` but kept as a guard; the flush skips the `lease_charge` so the caller's `hold_release` still runs). The `policy_id` is the row the flush fell back to; `NULL` on the missing-at-flush branch. History is retained forever (never truncated on ack); operator visibility rides on the aggregate.

| column | type | notes |
|---|---|---|
| `id` | `uuid` PK (`gen_random_uuid()`) | |
| `kind` | `text` NOT NULL | one of `policy_stale_fallback`, `policy_missing_at_flush`. `CHECK` constraint pins the domain so the C# enum and SQL never drift |
| `lease_id` | `uuid` | the lease the flush was billing when the fallback fired; nullable when the caller can't provide one |
| `policy_id` | `uuid` → `platform_policy(id)` | the row the flush split against on the stale branch; `NULL` on the missing branch |
| `occurred_at` | `timestamptz` NOT NULL DEFAULT `now()` | wall clock when the flush observed the fallback |

Index: `billing_incidents(occurred_at DESC)` backs the `MAX + ORDER BY DESC LIMIT 1` aggregate the admin overview reads.

### `operational_state`

Single-row table of one-off operator watermarks. Enforced single-row via a `smallint` primary key with `CHECK (id = 1)` and a seed `INSERT ... ON CONFLICT DO NOTHING` in the migration; a second insert is a PK conflict, and re-applying the migration is a no-op. First user: `policy_fallback_ack_at`, the last acknowledgement recorded by `POST /v1/admin/policy/fallback/ack`.

| column | type | notes |
|---|---|---|
| `id` | `smallint` PK DEFAULT `1` CHECK (`id = 1`) | pins the table to one row |
| `policy_fallback_ack_at` | `timestamptz` | last admin ack; the overview aggregates `billing_incidents` rows with `occurred_at > COALESCE(policy_fallback_ack_at, 'epoch'::timestamptz)` |

**Reader.** `IPolicyFallbackStore.GetAggregateAsync` returns `{ count, last_at, last_policy_id, ack_at }` for the post-ack window and backs `GET /v1/admin/overview`'s `platform_policy` block (`API.md` §8). A non-zero `count` flips overview `health` to `policy_fallback` (ranked below `ledger_drift`).

**Ack.** `POST /v1/admin/policy/fallback/ack` shifts `policy_fallback_ack_at` to `now`, so the badge clears while the incident history stays intact. Audited as `policy.fallback_ack` with the aggregate that was cleared in the meta. A fresh fallback recorded after the watermark re-flips the badge -- an ack is not a silence.

**In-memory dev mode.** The DB-less boot binds `InMemoryPolicyFallbackStore` behind the same interface, so the full flow (record on flush, aggregate on overview, ack endpoint) works without Postgres. State resets on restart, matching every other in-memory repository.
```
