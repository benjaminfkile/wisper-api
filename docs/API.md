...
| Method | Path | Notes |
|---|---|---|
| GET | `/v1/admin/overview` | `{ currency, revenue_cents (platform_revenue balance), wallet_liability_cents, host_earnings_cents, active_lease_count, host_count, online_host_count, user_count, health, ledger_reconcile, platform_policy }`; `ledger_reconcile` is `{ ran_at, accounts_checked, drift_account_count, total_absolute_drift_cents, has_drift }` from the most recent scheduled reconciliation pass on this instance (`DATA_MODEL.md` §7e; `ran_at` is null until the first pass completes). `platform_policy` is `{ active, fallback_count, last_fallback_at, last_fallback_policy_id }` (task #206, `PAYMENTS.md` §4): `active` reports whether an active `platform_policy` row currently resolves; the `fallback_*` fields count how often the metering flush had to fall back this boot (stale: no active row but a version exists, MeteringService logs `billing.policy.stale_fallback`; missing: no row at all, `billing.policy.missing_at_flush`) with the last policy id it used (null on the missing branch). Counters are process-local (each instance reports its own). `health` reads `"ok"` normally, flips to `"ledger_drift"` when the last observed reconcile pass saw non-zero drift, and to `"policy_fallback"` when the flush has recorded any fallback since boot on this instance; `ledger_drift` outranks `policy_fallback` when both are true. No other downstream is probed today |
