...
- **Roles are additive** (`DESIGN.md` §10), sourced from Cognito **groups** (`consumer`, `host`, `admin`). Every authenticated user is implicitly `consumer`; `host`/`admin` are added. Endpoint tables below mark the **minimum** role.
...
- **No third-party API keys / OAuth-app model** yet — clients are the first-party Next.js apps using Cognito JWTs. A programmatic API-key surface for external integrators is additive (issue keys → map to a service principal) when there's demand.
