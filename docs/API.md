...
| Method | Path | Notes |
|---|---|---|
| GET | `/v1/admin/overview` | `{ currency, revenue_cents (platform_revenue balance), wallet_liability_cents, host_earnings_cents, active_lease_count, host_count, online_host_count, user_count, health, ledger_reconcile, platform_policy }`; `ledger_reconcile` is `{ ran_at, accounts_checked, drift_account_count, total_absolute_drift_cents, has_drift }` from the most recent scheduled reconciliation pass on this instance (`DATA_MODEL.md` §7e; `ran_at` is null until the first pass completes). `platform_policy` is `{ active, fallback_count, last_fallback_at, last_fallback_policy_id, ack_at }` (task #206, task #210, `PAYMENTS.md` §4, `DATA_MODEL.md` §17): `active` reports whether an active `platform_policy` row currently resolves; the `fallback_*` fields aggregate the persistent `billing_incidents` journal after the ack watermark on `operational_state.policy_fallback_ack_at` (task #210 replaces the boot-local counter: the metering flush writes one row per observed fallback so the badge survives restarts and reads the same on every instance). Counts stale fallbacks (`billing.policy.stale_fallback`, an active row was missing but at least one version existed) plus missing-at-flush fallbacks (`billing.policy.missing_at_flush`, no row at all) with the newest policy id observed (null on the missing branch). `ack_at` is the last acknowledgement watermark or `null` when none has run. `health` reads `"ok"` normally, flips to `"ledger_drift"` when the last observed reconcile pass saw non-zero drift, and to `"policy_fallback"` while any unacknowledged fallback is recorded; `ledger_drift` outranks `policy_fallback` when both are true. No other downstream is probed today |
| POST | `/v1/admin/policy/fallback/ack` | acknowledges the platform-policy fallback signal (task #210, `PAYMENTS.md` §4, `DATA_MODEL.md` §17). Body is not required. Shifts the ack watermark on `operational_state.policy_fallback_ack_at` to `now`, so the next `GET /overview` reports `platform_policy.fallback_count = 0` and `health = "ok"` until a fresh fallback is recorded. The `billing_incidents` history is retained -- an ack is a badge clear, not a delete. Response: `{ acknowledged_count, acknowledged_last_fallback_at, acknowledged_last_fallback_policy_id, ack_at }` (the aggregate that was cleared plus the new watermark). Audited as `policy.fallback_ack` with the acknowledged aggregate in `meta` (task #210) |
