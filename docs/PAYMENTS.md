...
**Operator visibility on the fallback (task #206, task #210).** Every fallback the flush observes also writes a row to `billing_incidents` (`DATA_MODEL.md` §17): `kind` (`policy_stale_fallback` or `policy_missing_at_flush`), the `lease_id`, the `policy_id` the flush fell back to (`NULL` on the missing branch), and `occurred_at`. `GET /v1/admin/overview` aggregates the durable journal after the ack watermark on `operational_state.policy_fallback_ack_at` and reports `platform_policy.fallback_count / last_fallback_at / last_fallback_policy_id / ack_at`; a non-zero count flips `health` to `policy_fallback` (ranked below `ledger_drift`). Task #210 replaces the earlier boot-local counter so the badge survives restarts and reads the same on every instance -- previously only the instance whose meter ticked the offending flush knew about the incident. `POST /v1/admin/policy/fallback/ack` shifts the watermark to `now`, clearing the badge without erasing history; the ack is audited as `policy.fallback_ack` carrying the aggregate that was cleared, and a fresh fallback recorded after the watermark re-arms the signal (an ack is a badge clear, not a silence).
