...
**Billing must never throw for lack of a policy (task #184).** Every `lease_charge` splits by `platform_policy.fee_bps` (`DATA_MODEL.md` §11), so a paid lease with no active policy row would fail its metering flush and run unbilled. Two guard rails now prevent that: migration `0017_DefaultPlatformPolicy` seeds a conservative default row on an empty table (`fee_bps = 0`, every cap NULL, via a `NOT EXISTS` insert so it never overrides an admin-published version); and `POST /v1/leases` reads the active policy up front and refuses the create with a clear error if there is none, before any wallet hold or tunnel frame. The DB-less dev-mode boot runs the same seed at startup via `SeedInMemoryDefaultsAsync`.
